...
{ private const string _potionSlotsKey = "PotionSlots";
    ...
    { new DynamicVar(_potionSlotsKey, 1m), new DynamicVar(_potionCountKey, 1m) };
    public override async Task AfterObtained()
    { await PlayerCmd.GainMaxPotionCount(base.DynamicVars[_potionSlotsKey].IntValue, base.Owner); }
    public override async Task BeforeCombatStart() 
    {
        Flash();
        List<PotionModel> list = PotionFactory.CreateRandomPotionsOutOfCombat(base.Owner, base.DynamicVars[_potionCountKey].IntValue, base.Owner.RunState.Rng.CombatPotionGeneration);
